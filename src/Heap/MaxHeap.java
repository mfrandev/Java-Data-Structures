...
public class MaxHeap<T extends Comparable<T>> implements Heap<T> {
    ...
    public static final int INITIAL_HEAP_SIZE = 17;
    ...
    /**
     * Constructor for the max heap, start off with no elements
     */
    public MaxHeap() {
     ...
     */
    public MaxHeap(boolean testMode) {
        data = (T[]) new Comparable[INITIAL_HEAP_SIZE];
        length = 0;
    ...
    /**
     * Add an element to the heap while maintaining the max heap's properties
     */
    @Override
    ...
    /**
     * Move a large element up the heap to restore the max-heap property
     */
    public void propagateUp() {
        ...
        // While the max-heap property does not hold
    ...
    /**
     * Move the root element down the heap to restore the max-heap property
     */
    public void propagateDown() {
        ...
        while(hasLeftChild(index)) {
            ...
            // Save the index of the larger value between the left and right children
            int largest = leftChildIndex(index);

            // Determine if the right child is larger
            if(hasRightChild(index)) {
                largest = data[largest].compareTo(data[rightChildIndex(index)]) > 0 ? largest : rightChildIndex(index);
            }
            
            // If the max-heap property is violated between the largest and index, swap them
            if(data[index].compareTo(data[largest]) < 0) swap(index, largest);

            // Otherwise max-heap is valid
            else break;
     ...
     * @param index int: node to determaxe if a parent exists
     ...
     * @param index int: node to determaxe if a left child exists
     ...
     * @param index int: node to determaxe if a right child exists
